...
<nav class="{{ $hidden }} border-b border-blue-600/[.08] flex justify-between items-center px-6 py-4 w-full bg-white shadow">
    <div class="grid grid-cols-[200px_100px_100px_200px] gap-4 items-center">
        <h1 class="font-bold text-2xl text-blac">BeriAspirasi</h1>
        <a href="{{ route('aspirasi.index') }}">
            <div class="items-center py-2.5 rounded-xl cursor-pointer transition-all text-[11px] flex flex-row font-semibold gap-1">
                <x-heroicon-o-home class="w-6 h-6 {{ request()->routeIs('aspirasi.index') ? 'text-blue-500' : 'text-gray-600' }}"/>
                <span class="{{ request()->routeIs('aspirasi.index') ? 'text-blue-500' : 'text-gray-600' }} text-base">Beranda</span>
            </div>
        </a>
        <a href="{{ route('aspirasi.histori') }}">
            <div class="relative flex flex-rowl items-center py-2.5 rounded-xl cursor-pointer transition-all text-[11px] font-semibold gap-1">
                ...
                <span class="{{ request()->routeIs('aspirasi.histori') ? 'text-blue-500' : 'text-gray-600' }} text-base">Histori</span>
            </div>
        </a>

        <a href="{{ route('aspirasi.feedback') }}">
            <div class="flex flex-row items-center py-2.5 rounded-xl cursor-pointer transition-all text-[11px] font-semibold gap-1">
                <x-heroicon-o-chat-bubble-bottom-center class="w-6 h-6 {{ request()->routeIs('aspirasi.feedback') ? 'text-blue-500' : 'text-gray-600' }}"/>
                <span class="{{ request()->routeIs('aspirasi.feedback') ? 'text-blue-500' : 'text-gray-600' }} text-base">Umpan Balik</span>
            </div>
        </a>
    </div>
        ...
        <form action="{{ route('logout') }}" method="POST">
            ...
            <div x-data="{ open: false }">
                ...
                <button
                    type="button"
                    @click="open = true"
                    class="flex items-center px-4 py-2 rounded-xl text-base font-semibold text-gray-600 border border-white/20 hover:bg-white/20 transition-all cursor-pointer"
                >
